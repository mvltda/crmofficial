...
if(isset($_GET['refQuotations']))
{
	...
	try {
		$sql = "SELECT 
			af_quotations.id AS quotations_id
			, af_clients.name AS client_full_name
			, geo_departments.name AS department
			, geo_citys.name AS city 
			, af_quotations.id AS quotation_number
			, af_quotations.create AS quotations_clients_create
			, af_quotations.update AS quotations_clients_update 
			, af_quotations.values AS listvalues
			, af_quotations.validity AS validity
			...
			FROM af_quotations 
			LEFT JOIN af_requests 
				ON af_quotations.request = af_requests.id 
			LEFT JOIN af_clients 
				ON af_quotations.client = af_clients.id 
			LEFT JOIN geo_departments
				ON af_clients.address_principal_department = geo_departments.id 
			LEFT JOIN geo_citys
				ON af_clients.address_principal_city = geo_citys.id 
			LEFT JOIN contacts 
				ON af_requests.contact = contacts.id 
			LEFT JOIN config_options 
				ON config_options.name IN ('proposalLetter')
			WHERE af_quotations.id='{$idQuotations}';";
		...
		if(isset($result[0]['quotations_id'])){
			...
			$QC_ProyectName = "Propuesta {$infoData['quotations_id']}";
